        ...
        if(dom[0] === 'tieba.baidu.com' && dom.length>1){
            if(web === 'http'){
                ...
                http.get(url , function(res){
                    if(func === 'simple'){
                        tieba(url, res, txtMsg, page);
                    }else if(func === 'first'){
                        firstPage(url, res, txtMsg, page);
                    }else{
    ...
    res.on('end' , function(){
        ...
        if(tiebaName === ''){
            ...
            if(tieziName !== 'undefined' && tiebaName){
                ...
                let people = $('.d_name .j_user_card'), sayContent = $('.j_d_post_content').text().split("            "), txtNum = 1+30*(page-1);
